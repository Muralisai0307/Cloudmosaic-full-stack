...
function Careers() {
  ...
  const handleNotifySubmit = async (e) => {
    ...
    try {
      ...
      if (err.errors && Object.keys(err.errors).length > 0) {
        ...
        error(`Subscription failed: ${errorMsgs}`);
      } else {
  ...
  const handleApplySubmit = async (e) => {
    ...
    try {
      ...
      if (applicantCoverLetter) {
        formData.append('cover_letter', applicantCoverLetter);
      }
      ...
      if (err.errors && Object.keys(err.errors).length > 0) {
        const errorMsgs = Object.values(err.errors).flat().join(' ');
        error(`Failed to submit: ${errorMsgs}`);
      } else {
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href="https://cloudmosaic.ai/careers" />
      </Helmet>
      <Header />
          ...
          {jobs.length > 0 ? (
            <div className="jobs-list">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '2rem' }}>
                {jobs.map(job => (
                  <div key={job.id} style={{ padding: '1.5rem', background: 'var(--surface-color)', borderRadius: '8px', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <h3 style={{ margin: 0 }}>{job.title}</h3>
                      <p style={{ margin: '0.5rem 0 0 0', color: 'var(--text-secondary)' }}>Remote • Full Time</p>
                    </div>
